...
new_workflow = '''async function triggerDownstreamWorkflow(doc) {
    try {
        ...
        await loadDbFromBackend(); // refresh the app state
    } catch (e) {
        console.error("Error triggering downstream workflow:", e);
    }
}'''

content = re.sub(r'function triggerDownstreamWorkflow\(doc\) \{[\s\S]*?(?=\nfunction pollDocumentStatus|\nfunction fallbackLocalUpload|\nfunction populateExtractedData)', new_workflow + '\n\n', content)
content = re.sub(r'const SAMPLE_CLINICAL_DOCS = \{[\s\S]*?\}\};\n', '', content)
content = re.sub(r'const SAMPLE_POLICY_DOCS = \{[\s\S]*?\}\};\n', '', content)
content = re.sub(r'function loadSampleIntake\(type\) \{[\s\S]*?(?=\nfunction submitIntake|\nfunction resetIntake)', '', content)
content = re.sub(r'function loadClinicalSample\(type\) \{[\s\S]*?(?=\nfunction submitClinicalIntake|\nfunction resetClinicalIntake)', '', content)
content = re.sub(r'function loadPolicySample\(type\) \{[\s\S]*?(?=\nfunction submitPolicyIntake|\nfunction resetPolicyIntake)', '', content)

# Also remove window.loadSampleIntake... exports
content = re.sub(r'\s*window\.loadSampleIntake = loadSampleIntake;', '', content)
content = re.sub(r'\s*window\.loadClinicalSample = loadClinicalSample;', '', content)
content = re.sub(r'\s*window\.loadPolicySample = loadPolicySample;', '', content)
